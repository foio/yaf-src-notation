...
yaf_loader_t * yaf_loader_instance(yaf_loader_t *this_ptr, char *library_path, char *global_path TSRMLS_DC) {
	 ...
	 */
		//如果设置了library_path,则更新yaf_loader的_library属性
		if (library_path) {
		...
		//如果设置了global_path,则更新yaf_loader的_global_library属性
		if (global_path) {
	...
	//global_path和library_path都为null直接返回NULL
	...
	//设置对象的library_path和global_path的属性,涉及到字符串对象的释放
