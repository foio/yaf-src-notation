...
//定义类的构造函数的参数 pass_rest_by_reference=0, return_reference=0, required_num_args=1
...
PHP_METHOD(yaf_application, __construct) {
	...
	//当app不为空时表示已经有其他的应用存在，每一个Yaf_Application只能对应唯一的应用
	if (!ZVAL_IS_NULL(app)) {
		yaf_trigger_error(YAF_ERR_STARTUP_FAILED TSRMLS_CC, "Only one application can be initialized");
		RETURN_FALSE;
	}

	//获得当前对象的指针, #define getThis() (this_ptr)
	self = getThis();

	//读取参数，config为必选参数，section为可选参数(用于指定选取配置文件的一段)，|之后的参数为可选
	...
	if (!section || Z_TYPE_P(section) != IS_STRING || !Z_STRLEN_P(section)) {
		//如果section为空或者不符合要求，设置section为全局变量yaf的environ值
		MAKE_STD_ZVAL(section);
		ZVAL_STRING(section, YAF_G(environ), 0);
		zconfig = yaf_config_instance(NULL, config, section TSRMLS_CC);//创建yaf_config对象
		efree(section);
	} else {
		//如果section符合要求，根据section创建yaf_config对象，如果开启ini配置缓存会首先从缓存中读取
	...
	//创建dispacher对象,并设置默认router,model,controller,action
	...
	//设置dispacher的_request属性
	yaf_dispatcher_set_request(zdispatcher, request TSRMLS_CC);
	//设置yaf_application的config,和dispacher属性
	...
	if (YAF_G(local_library)) {
		//根据local_library所配置的路径生产yaf_loader
		...
		//使用默认的library作为local_library，并生成yaf_loader
		char *local_library;
	...
	//设置yaf_application的运行状态标志:_running和环境标志_environ
...
*/
//注册Yaf_Application的方法
zend_function_entry yaf_application_methods[] = {
	PHP_ME(yaf_application, __construct, 		yaf_application_construct_arginfo, 	ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)//注册类的构造函数,访问级别为public,参数定义在yaf_application_construct_arginfo中
...
*/
YAF_STARTUP_FUNCTION(application) {//Yaf_Application的类定义函数，在模块初始化函数MINT中调用
	zend_class_entry ce;//zend类容器,php扩展类定义相关知识请参考http://www.walu.cc/phpbook/preface.md
	YAF_INIT_CLASS_ENTRY(ce, "Yaf_Application", "Yaf\\Application", yaf_application_methods);//注册类的方法

	yaf_application_ce = zend_register_internal_class_ex(&ce, NULL, NULL TSRMLS_CC);//注册类
	yaf_application_ce->ce_flags |= ZEND_ACC_FINAL_CLASS;//设置类为final，禁止继承
	
	//定义Yaf_Application的属性,属性名定义在yaf_application.h,YAF_APPLICATION_PROPERTY_NAME_CONFIG为config,以下属性定义语句原理类似,属全部定义为protected访问级别
