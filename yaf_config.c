 ...
 */
//yaf框架的配置文件缓存读取函数，当yaf.cache_config开启时在yaf_application的构造函数中调用
static yaf_config_t * yaf_config_ini_unserialize(yaf_config_t *this_ptr, zval *filename, zval *section TSRMLS_DC) {
	char *key;
	uint len;
	...
	//根据filename和section生成hashtab的key
	...
	if (zend_hash_find(YAF_G(configs), key, len + 1, (void **)&ppval) == SUCCESS) {
		...
		if (yaf_config_ini_modified(filename, (*ppval)->ctime TSRMLS_CC)) {
			...
			MAKE_STD_ZVAL(props);
			array_init(props);
			yaf_config_copy_losable(Z_ARRVAL_P(props), (*ppval)->data TSRMLS_CC);//从hashtab中取出
			efree(key);
			/* tricky way */
			Z_SET_REFCOUNT_P(props, 0);
			return yaf_config_ini_instance(this_ptr, props, section TSRMLS_CC);//根据从hashtab取出的值设置配置对象并返回
		}
		efree(key);
	}
 ...
 */
//该函数用于从配置文件或者数组中读取配置信息，在yaf_application的构造函数中调用
yaf_config_t * yaf_config_instance(yaf_config_t *this_ptr, zval *arg1, zval *arg2 TSRMLS_DC) {
	...
	//yaf的错误处理函数，根据配置信息，可以抛出异常或者设置全局错误状态
